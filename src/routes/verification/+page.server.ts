	...
	default: async (request) => {
		...
		try {
			...
			if (userError && !(userError?.code === 'PGRST116'))
				return fail(500, { form, error: 'Error fetching user' });
			...
			switch (form.data.step) {
				...
				case 0:
					...
					if (!user || !user.email_confirmed_at) {
						const { data: newUser, error: signUpError } = await supabaseAdmin.auth.signUp({
							...
							password: P,
							options: { data: { name: form.data.name, step: form.data.step } }
						});
						if (!newUser.user || signUpError)
							return fail(500, { form, error: `Error during sign up: ${signUpError}` });
					} else {
						// @ts-ignore
				...
				case 1:
					...
					if (form.data.phone.otp.length === 6) {
						...
						const response = await supabaseAdmin.auth.verifyOtp({
							phone,
							token: form.data.phone.otp,
							type: 'phone_change'
						});
					...
					if (auth.error) {
						...
						return setError(
							form,
							'phone.number',
							'Unknown Error | Please try again later or contact support support@0xargus.org',
							{ status: 500 }
						);
					}
					...
					if (updateUser.error) {
						...
						return setError(form, 'phone.number', updateUser.error?.message, {
							overwrite: true,
							status: 403
						});
					}
