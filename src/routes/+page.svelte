<script lang="ts">
	...
	// ICONS
	import { ArrowRightIcon, BotIcon, CandlestickChartIcon, EyeIcon, MoveRightIcon } from 'lucide-svelte';
	...
	import Circles from '$lib/assets/circles@2x.png';
	import SupportCardImage from '$lib/assets/SupportCardImage.png?enhanced';
	import Card1 from '$lib/assets/FeatureCard1.png?enhanced';
	...
	const howItWorkCards = [
		...
		},
		{
			...
			img: 'src/lib/assets/FeatureCard2.png'
		},
		{
			...
			img: 'src/lib/assets/FeatureCard3.png'
		}
	];
	const featureCards = [
...
<Modal {...styles} classWindow="p-2 bg-bgPrimary rounded-3xl">
	...
	<GridAndDotBackgroundsSmallGrid>
		...
		<Section id="about" class="sm:mt-12">
			...
			<!-- <video class="-sm:mt-[650px] 0mt-[450px] inset-0 z-[1000] mx-auto mt-40 max-h-[800px] w-full" autoplay controls={false} muted loop playsinline>
				<source src="/coins.mov" type="video/quicktime" />
				<source src="/coins.webm" type="video/webm" />
				Your browser does not support the video tag.
			</video> -->

			<!-- FEATURES SECTION-->
			<!-- <div class="mt-[250px] sm:mt-[500px] md:mt-[750px]" /> -->
			<SectionHeader id="features" title="Bringing Security into the Mainstream" subtitle="The first ever safety layer middleware. Giving you the ultimate protection for your DeFi investments" />
			...
			<div class="flex flex-col gap-4 gap-y-8 sm:gap-8">
				...
				<div
					class="relative grid min-h-64 w-full grid-rows-2 items-center justify-center gap-y-8 overflow-hidden rounded-xl bg-gradient-to-r from-indigo-600/50 via-indigo-600/60 to-indigo-600/40 px-4 py-12 shadow-xl sm:min-h-96 md:grid-cols-2 md:grid-rows-1 md:py-24">
					<!-- TODO: Fix enhancement with responsive grid -->
					<img class="absolute" src={Circles} alt="bg radial" />
					<div class="z-20 h-full space-y-6 px-4 text-white sm:px-12">
	...
	<Section id="howitworks" className="relative pb-0">
		...
		<SectionHeader title="Crypto Safeguards: The Argus Approach" subtitle="Discover the steps involved in safeguarding your digital assets with our cutting-edge technology" />
			...
			{#each howItWorkCards as { desc, img, title }, index}
				...
				<div
					...
					{#if index === 0}
						...
						<enhanced:img loading="lazy" class="absolute inset-x-0 bottom-0 z-[50] h-64 rounded-xl sm:h-96" src={img} alt="img" />
					{:else}
						<enhanced:img loading="lazy" style="background-image: url('{img}');" class="z-[50] m-auto h-64 w-full rounded-xl bg-contain bg-center bg-no-repeat sm:h-96 sm:max-w-[30rem]" />
					{/if}
				</div>
			{/each}
		</div>
