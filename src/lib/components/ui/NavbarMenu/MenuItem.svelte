...
<div
	...
	<Motion let:motion transition={{ duration: 0.3 }}>
		...
		<Hamburger
			class="min-h-12 min-w-12 cursor-pointer rounded-full p-2 text-neutral-100 duration-500  ease-in-out hover:bg-white/10 hover:text-indigo-500"
			bind:open={active} />
	</Motion>
	{#if active !== null}
		<Motion
				...
				{#if active === item}
					<div class="absolute left-1/2 -translate-x-1/2 transform pt-4">
						<Motion let:motion {transition} layoutId="active">
							<div
								use:motion
								class="overflow-hidden rounded-2xl border border-white/[0.2] bg-neutral-900 shadow-xl backdrop-blur-sm">
